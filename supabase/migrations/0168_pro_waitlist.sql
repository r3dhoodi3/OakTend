-- =============================================================================
-- OakTend - pro waitlist + a service-role escape from the home cap (0168)
-- RUN THIS AGAINST THE LIVE DATABASE (Supabase SQL editor); editing repo SQL
-- alone does NOT change the already-deployed database.
--
-- WHY: HOMEOWNER PREVIEW MODE (Landen addendum 3). Until the lawyer review is
-- finished, OakTend ships homeowner-only: every homeowner feature is on and
-- free, nothing anywhere is chargeable, and the contractor side is closed
-- behind a "coming soon" page. The switch is the environment variable
-- NEXT_PUBLIC_PREVIEW_MODE=homeowner (src/lib/previewMode.ts); see
-- docs/GO-LIVE-WIRING.md. Two things in this file are what that mode needs
-- from the database, and BOTH are written so that pasting them changes
-- nothing at all while preview mode is off.
--
-- PART 1 - public.pro_waitlist. The closed contractor doors (/pros,
--   /contractor-signup, /pro/onboarding, the pro role choice) all render one
--   page with an email-capture form on it. This is where that email lands.
--   RLS on, NO policies, grants revoked: service role only. It is a list of
--   contractors' email addresses, so `anon` must never be able to read it.
--
-- PART 2 - one clause added to public.enforce_properties_home_cap (0108).
--   In preview every homeowner is treated as OakTend Plus, which includes up
--   to 5 homes, but that trigger derives Plus from the subscriptions table
--   directly and a preview homeowner has no subscriptions row - so it would
--   still refuse their SECOND home with "free accounts can track 1 home".
--   The added clause lets a SERVICE-ROLE insert through, and
--   claimPropertyAction routes its insert through the admin client only when
--   preview mode is on. Every session-client insert - which is every insert
--   a normal deploy makes - hits the identical 1/5 count it always has.
--
...
-- binding. Re-creating it here would only risk dropping it on a half-applied
-- run.
