-- =============================================================================
-- PASTE-ME-0170-remove-applicant-cap-2026-09-18.sql   (built 2026-09-18)
-- Live-paste copy of migration 0170 (remove_applicant_cap). Run this whole
-- file in the Supabase SQL editor against the live database. Below the
-- PRECHECK it is byte for byte supabase/migrations/0170_remove_applicant_cap.sql.
--
-- Live is expected to be through 0169 (the 2026-09-16 wave). The only real
-- dependency is 0165: this file re-creates public.apply_to_lead() from 0165's
-- body, so 0165 has to be applied first or the internal-accounts guard would
-- be silently rolled back. The PRECHECK below refuses in that case and
-- changes nothing.
--
-- WHAT IT DOES. Removes the 3-applicant cap from public.apply_to_lead():
-- the count of live (non-refunded) applications no longer refuses a fourth
-- pro with 'Job is full'. Founder decision 2026-09-16 (addendum 6): the
-- homeowner sees every applicant in one list, compares them, and picks.
-- Contact details still stay private until that pick (choose_applicant,
-- untouched here).
--
-- WHAT IT DOES NOT DO. No new objects, no data migration, no row is written.
-- Every other guard and every charge path in the function is 0165's body
-- unchanged: the chargeback freeze, the service-area gates, the pricing, the
-- insurance gate, the self-apply guard, the internal-accounts guard, the
-- block check, the relationship rule, the row lock, the wallet lock, and both
-- inserts. CREATE OR REPLACE with the same signature, so the EXECUTE grant to
-- `authenticated` is preserved and the whole file is safe to re-run.
--
-- APPLY ORDER: 0165 -> (0166, 0167, 0168, 0169 in any order) -> this file.
-- =============================================================================

-- ---- PRECHECK: refuse to run against a database that is not ready ----------
-- apply_to_lead is re-created below from 0165's body. If live is BEHIND 0165,
-- pasting this would drop the internal-accounts guard (a real homeowner could
-- then transact with a team test pro, and vice versa), so it is fingerprinted
-- first and nothing is applied unless it matches.
do $precheck$
begin
  if not exists (
  ...
  if not exists (
    ...
    raise exception 'PRECHECK: public.users.is_internal (migration 0165) is missing, so this database has not run the internal-accounts migration. Run supabase/PASTE-ME-0165-internal-accounts-2026-09-12.sql FIRST, then this file. NOTHING WAS CHANGED.';
  end if;
  if not exists (
    select 1 from pg_proc
    where proname = 'is_internal_contractor' and pronamespace = 'public'::regnamespace
  ) then
    raise exception 'PRECHECK: public.is_internal_contractor() (migration 0165) is missing. Run supabase/PASTE-ME-0165-internal-accounts-2026-09-12.sql FIRST, then this file. NOTHING WAS CHANGED.';
  end if;
  if not exists (
    select 1 from pg_proc
    where proname = 'apply_to_lead' and pronamespace = 'public'::regnamespace
    ...
    raise exception 'PRECHECK: public.apply_to_lead() is not on 0165''s body (no internal-accounts guard). The body below IS 0165''s minus the applicant cap, so applying it now would roll back whatever newer body is live. Apply 0165 before this file. Nothing was changed.';
  end if;
end
$precheck$;
...
-- ---- VERIFY -----------------------------------------------------------------
-- select pg_get_functiondef('public.apply_to_lead(uuid, text)'::regprocedure)
--          like '%Job is full%' as still_capped;
-- -- expect false (the cap is gone).
--
-- With the same pg_get_functiondef(...) like '%...%' shape, each of these is
-- expected TRUE - they are the guards and charge paths 0170 must not have
-- touched:
--   '%internal_account%'                        (0165 internal accounts)
--   '%You cannot apply to your own job.%'        (0161 SEC-1)
--   '%Insurance required for big jobs%'          (0153 big-job gate)
--   '%has_open_chargeback%'                      (0132 dispute freeze)
--   '%blocked_between%'                          (0138 blocks)
--   '%Already working with this homeowner%'      (0060 relationship rule)
--   '%from contractor_leads where id = p_lead%'  (the row lock, kept)
--   '%pro_lead_fee_cents%'                       (0149 pricing)
--   '%major_lead_price_cents%'                   (0113 intro price)
--
-- select obj_description('public.apply_to_lead(uuid, text)'::regprocedure, 'pg_proc');
-- -- expect the comment to say "latest body 0170".
--
-- select has_function_privilege('authenticated', 'public.apply_to_lead(uuid, text)', 'execute');
-- -- expect true (CREATE OR REPLACE kept 0012's grant).
--
-- A real check, once a job has three live applicants: a fourth pro applying
-- gets a normal result instead of 'Job is full'.
-- select lead_id, count(*) filter (where refunded_at is null) as live_applications
--   from public.lead_applications group by lead_id order by live_applications desc limit 5;
