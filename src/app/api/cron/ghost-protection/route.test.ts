...
// Ghost protection pays a refund on a retired fee model (per-lead apply
// fees), so this cron is paused behind RETIRED_PRO_PROGRAMS_PAUSED
...
// pre-existing sweep, once the success-fee rebuild ships and the flag flips
// back to false).
...
describe("ghost-protection cron while RETIRED_PRO_PROGRAMS_PAUSED is true", () => {
  ...
  it("skips the sweep and returns the paused-skip shape, touching no DB", async () => {
    const { GET } = await import("./route");
...
describe("ghost-protection cron once unpaused", () => {
  it("falls through to the real sweep instead of the skip shape", async () => {
    paused = false;
    const { GET } = await import("./route");
    const res = await GET(authed());
    expect(res.status).toBe(200);
    ...
    expect(body).toEqual({ refunded: 0, directRefunded: 0 });
